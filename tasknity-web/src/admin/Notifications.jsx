...
export default function Notifications() {
  ...
  const checkAndCreateNotifications = async () => {
    try {
      // Get all tasks that are due within 1 day (24 hours from now)
      ...
      const { data: existingNotifications } = await supabase
        ...
        .eq("type", "document_overdue");

      const existingSet = new Set(
        existingNotifications?.map(n => `${n.task_id}-${n.user_id}`) || []
      );

      // Create notifications for tasks that don't have one yet
      const newNotifications = [];
      for (const task of upcomingTasks || []) {
        const notificationKey = `${task.id}-${task.assigned_to}`;
  ...
  if (notifications.length === 0) {
    return <div className="text-gray-500 text-sm">No pending document notifications</div>;
  }
